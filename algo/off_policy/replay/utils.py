...
def init_buffer(buffer, capacity, state_space, action_dim, has_priority, state_extra_space=0):
    ...
    target_buffer.update({
        'state': np.zeros((capacity + state_extra_space, *state_space), dtype=state_dtype),
...
def add_buffer(buffer, idx, state, action, reward, done, n_steps, gamma):
