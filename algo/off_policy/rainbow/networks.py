...
class QNets(Base):
    ...
    def _build_graph(self):
        ...
        self.Q_dist = self._dense_net(x, self.num_actions, name='main')
        self.next_Q_dist = self._dense_net(next_x, self.num_actions, name='main')
        self.target_Q_dist = self._dense_net(next_x, self.num_actions, name='target')
    ...
    def _conv_net(self, state):
        ...
        with tf.variable_scope('convnet'):
            ...
            x = self.conv_norm_activation(x, 64, 4, 2, padding='same', norm=None)      # (10, 10, 64)
            x = self.conv_norm_activation(x, 64, 3, 1, padding='same', norm=None)      # (10, 10, 64)
