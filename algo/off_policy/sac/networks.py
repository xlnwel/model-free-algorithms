...
class SoftPolicy(Module):
    ...
    def _build_graph(self):
        self.action_det, self.n_action_repetitions, self.action, self.logpi = self._build_policy(self.state, 'main', False)
        if self.has_target_net:
            _, self.next_n_action_repetitions, self.next_action, self.next_logpi = self._build_policy(self.next_state, 'target', False)
        else:
            _, self.next_n_action_repetitions, self.next_action, self.next_logpi = self._build_policy(self.next_state, 'main', True)

        self.action_repr = tf.concat([self.action, self.n_action_repetitions], 1)
        self.next_action_repr = tf.concat([self.next_action, self.next_n_action_repetitions], 1)
        self.action_det_repr = tf.concat([self.action_det, self.n_action_repetitions], 1)

        self.init_target_op, self.update_target_op = self._target_net_ops()

    def _build_policy(self, state, name, reuse):
        with tf.variable_scope(name, reuse=reuse):
            mean, logstd, action_det, n_action_repetitions = self._stochastic_policy_net(state, 
                                                                                        self.args['units'], 
                                                                                        self.action_dim, 
                                                                                        self.norm)
        ...
        return action_det, n_action_repetitions, action, logpi
    ...
    def _stochastic_policy_net(self, state, units, action_dim, norm, name='policy_net'):
        ...
        with tf.variable_scope(name):
            ...
            n_action_repetitions = self.dense(x, self.args['max_action_repetitions'], name='action_repetitions')
            n_action_repetitions = tf.nn.softmax(n_action_repetitions)

        return mean, logstd, mean, n_action_repetitions
...
class SoftQ(Module):
    ...
    def __init__(self, 
        ...
        self.actor_action_repr = actor.action_repr
        self.actor_next_action_repr = actor.next_action_repr
        self.norm = layer_norm if 'layernorm' in args and args['layernorm'] else None
    ...
    def _build_graph(self):
        ...
        with tf.variable_scope('main'):
            self.Q1 = Q_net(self.state, self.action_repr, False, 'Qnet1')
            self.Q2 = Q_net(self.state, self.action_repr, False, 'Qnet2')
            self.Q1_with_actor = Q_net(self.state, self.actor_action_repr, True, 'Qnet1')
            self.Q2_with_actor = Q_net(self.state, self.actor_action_repr, True, 'Qnet2')
            self.Q = tf.minimum(self.Q1, self.Q2, 'Q')
            self.Q_with_actor = tf.minimum(self.Q1_with_actor, self.Q2_with_actor, 'Q_with_actor')

        # target network
        with tf.variable_scope('target'):
            self.next_Q1_with_actor = Q_net(self.next_state, self.actor_next_action_repr, False, 'Qnet1_target')
            self.next_Q2_with_actor = Q_net(self.next_state, self.actor_next_action_repr, False, 'Qnet2_target')
