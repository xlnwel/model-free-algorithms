...
class Actor(Base):
    ...
    def _build_graph(self, **kwargs):
        self.action = self._actor(self.state)
...
class Critic(Base):
    ...
    def _plain_critic(self, state, action, reuse):
        self._reset_counter('dense_resnet')
        
        with tf.variable_scope('plain_net', reuse=reuse):
            x = self._dense(state, 512 - self.action_size, kernel_initializer=tf_utils.kaiming_initializer())
    ...
    def _distributional_critic(self, state, action, reuse):
        self._reset_counter('dense_resnet')
