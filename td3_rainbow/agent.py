...
class Agent(Model):
    ...
    def act(self, observation):
        ...
        action = self.sess.run(self.actor.action, feed_dict={self.actor.observation_ph: observation})
    ...
    def _prepare_data(self, buffer):
        ...
        # prepare data
        IS_ratio, (obs, action, reward, next_obs, done, steps) = samples
        ...
        data['action'] = action
        data['reward'] = reward
        data['next_observation'] = next_obs
        data['done'] = done
        data['steps'] = steps
    ...
    def _learn(self):
        ...
        for _ in range(self._critic_update_times-1):
            self.sess.run([self.critic_opt_op])
