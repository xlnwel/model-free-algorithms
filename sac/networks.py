...
class SoftPolicy(Base):
    ...
    def _build_graph(self):
        dist_params = self._stochastic_policy_net(self.state, self.args['units'], self.action_dim, 
                                                self.reuse, discrete=self.env.is_action_discrete)
...
class SoftV(Base):
    ...
    def _build_graph(self):
        self.V = self._V_net(self.state, self.args['units'], self.reuse, 'main')
        self.target_V = self._V_net(self.next_state, self.args['units'], self.reuse, 'target')  # target V use next state as input
...
class SoftQ(Base):
    ...
    """ Implementation """
    def _build_graph(self):
        self.Q1 = self._Q_net(self.state, self.args['units'], self.action, self.action_dim, self.reuse, name='Qnet1')
        self.Q1_with_actor = self._Q_net(self.state, self.args['units'], self.action, self.action_dim, True, name='Qnet1')
        self.Q2 = self._Q_net(self.state, self.args['units'], self.action, self.action_dim, self.reuse, name='Qnet2')
        self.Q2_with_actor = self._Q_net(self.state, self.args['units'], self.action, self.action_dim, True, name='Qnet2')
